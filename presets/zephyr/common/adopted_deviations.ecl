# Derived from https://github.com/zephyrproject-rtos/zephyr/blob/v4.3.0/cmake/sca/eclair/ECL/adopted_deviations.ecl
...
-config="^MC[A-Z0-9]\\.R7\\.2$",reports+={relied,adopted_report}
-config="^MC[A-Z0-9]\\.R10\\.3$",reports+={relied,adopted_report}
-config="^MC[A-Z0-9]\\.R10\\.6$",reports+={relied,adopted_report}
-config="^MC[A-Z0-9]\\.R12\\.1$",reports+={relied,adopted_report}
-doc_end
...
-config="^MC[A-Z0-9]\\.R7\\.2$",reports+={safe,"all_area(all_loc(any_exp(macro(^assert$))))"}
-doc_end
...
-config="^MC[A-Z0-9]\\.R2\\.3$",declarations+={safe,"loc(top(public()))"}

-doc="Library entry points not necessarily have to be referenced."
-config="^MC[A-Z0-9]\\.R2\\.1$",declarations+={safe,"loc(top(public()))"}

-doc="Library entry points not necessarily have to be referenced in more than one translation units."
-config="^MC[A-Z0-9]\\.R8\\.7$",declarations+={safe,"loc(top(public()))"}
...
-config="^MC[A-Z0-9]\\.R8\\.8$",declarations={relied, "^z_vrfy_.*$||^z_impl_.*$"}

-doc="Function hash, that is automatically generated, does not use the parameter \"len\" in all its definitions."
-config="^MC[A-Z0-9]\\.R2\\.7$",declarations+={relied,"context(^hash\\(const char\\*, size_t\\)$)&&name(len)"}

-doc="Function \"z_object_lookup\", that is automatically generated, uses single-statement bodies not enclosed in braces."
-config="^MC[A-Z0-9]\\.R15\\.6$",reports={relied, "all_area(context(^z_object_lookup\\(const char\\*, size_t\\)$))"}
...
-config="^MC[A-Z0-9]\\.R8\\.6$",declarations+={safe, "loc(top(file(^zephyr/build/zephyr/include/generated/.*$)))"}
